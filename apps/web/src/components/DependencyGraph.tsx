  ...
  return (
    <div className="p-4 border rounded bg-gray-50 flex overflow-x-auto gap-4 items-center">
      {nodes.map((node: any, i: number) => (
        <div key={node.id || node._id || i} className={`p-3 border rounded shadow-sm flex flex-col gap-2 min-w-[150px] bg-white ${node.status === 'broken' ? 'border-red-500' : node.status === 'at_risk' ? 'border-yellow-500' : 'border-green-500'}`}>
